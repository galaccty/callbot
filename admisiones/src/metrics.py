...
class Metrics:
    ...
    def estado_table(self) -> pd.DataFrame:
        """Tabla Estado / Cantidad con orden fijo Entregada -> Faltante."""
        if self.data.empty:
            return pd.DataFrame({"Estado": ["Entregada", "Faltante"],
                                 "Cantidad": [0, 0]})
        s = self.data["Estado_Entrega"].replace("", "Sin estado")
        counts: Dict[str, int] = {}
        for estado in ["Entregada", "Faltante"]:
            counts[estado] = int((s == estado).sum())
        for estado, n in s[~s.isin(["Entregada", "Faltante"])].value_counts().items():
            counts[estado] = int(n)
        return pd.DataFrame({"Estado": list(counts.keys()),
                             "Cantidad": list(counts.values())})
